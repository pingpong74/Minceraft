...
impl GpuContext {
    ...
    pub fn resize(&mut self, width: u32, height: u32, render_context: &RenderContext) {
        if width > 0 && height > 0 {
            self.sufrace_config.width = width;
            self.sufrace_config.height = height;
            self.surface
                .configure(&render_context.device, &self.sufrace_config);
...
impl Renderer {
    ...
    pub fn resize(&mut self, width: u32, height: u32) {
        self.gpu_context.resize(width, height, &self.render_context);
    }
    ...
    pub fn render(&mut self) -> anyhow::Result<()> {
        ...
        let mut encoder = self.render_context.create_encoder();
